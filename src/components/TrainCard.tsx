...
export default function TrainCard({ train, rank, fresh, minutesSaved }: { train: JourneyTrain; rank?: string; fresh: boolean; minutesSaved?: number | null }) {
  ...
  return <article className={`train-card${fresh ? "" : " stale"}`}>
    ...
    <dl className="details"><div><dt>Origin</dt><dd>{londonTime(train.expectedArrival)}</dd></div><div><dt>Platform</dt><dd className={train.platformConfirmed ? "platform-confirmed" : undefined}>{platform}</dd></div><div><dt>Destination</dt><dd aria-label={`Destination ETA: ${train.evidence === "live" ? "Live" : train.evidence === "estimate" ? "Estimated" : "Unavailable"}`}>{eta(train)}</dd></div></dl>
