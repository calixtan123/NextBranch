...
export default function Home() {
  ...
  const { data, loading, issue, fetchJourney, cooldownUntil, retryAfterSeconds } = routeRequest;
  const { data: departures, loading: departuresLoading, issue: departuresIssue, fetchDepartures, cooldownUntil: departuresCooldown, retryAfterSeconds: departuresRetryAfterSeconds } = departureRequest;
  ...
  const departureProblem = departuresIssue === "offline" ? "Live TfL data requires an internet connection." : departuresIssue === "upstream" ? "Live TfL data is temporarily unavailable. Try again shortly." : departuresIssue === "rate_limited" ? `Too many live requests. Try again in ${departuresRetryAfterSeconds ?? 30} seconds.` : "We couldn’t read the latest live prediction.";
  ...
  return <main>
    ...
    {view === "results" && issue && <div role="alert" className="warning">{issue === "offline" ? "Live TfL data requires an internet connection." : issue === "upstream" ? "Live TfL data is temporarily unavailable. Try again shortly." : issue === "rate_limited" ? `Too many live requests. Try again in ${retryAfterSeconds ?? 30} seconds.` : "We couldn’t read the latest live prediction."}{data && <><br /><strong>Last prediction — information may be stale.</strong></>} <button disabled={now !== null && now < cooldownUntil} onClick={() => void fetchJourney()}>Retry</button></div>}
